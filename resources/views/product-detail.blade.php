                                            ...
                                            <div class="product-info col-xs-12 col-md-7 col-sm-7">
                                                <div class="detail-description">
                                                    ...
                                                    <div class="option has-border d-lg-flex size-color">
                                                        ...
                                                        @foreach($childColor as $child)
                                                            <div>
                                                                <input class="child-color" type="radio" name="child-color" value="{{ $child->adon_id }}" onchange="updateGrandChildColors({{ $child->adon_id }})">
                                                                <label>{{ $child->adon_title }}</label>
                                                            </div>
                                                        @endforeach
                                                    </div>

{{--                                                    Showing the grandchild of color_items--}}
                                                    <div class="form-group grand-child-container" id="grandChildColorContainer" >
                                                        <!-- Grandchild Colors will be displayed here -->
                                                        <strong>Here</strong>
                                                    </div>
                                                        ...
                                                        <div class="row">
                                                            <div class="col-sm-4">
                                                                @foreach ($door_type_items as $item)
                                                                    <div class="radio">
                                                                        <label>
                                                                            <input type="radio" name="door_type" value="{{ $item->adon_item_id }}" data-price="{{ $item->adon_item_price }}" data-adon_item_name="{{ $item->adon_item_name }}">
                                                                            {{ $item->adon_item_name }}
                                                                        </label>
                                                                    </div>
                                                                @endforeach
...
{{--                                                        SHOWING THE SIZES--}}
                                                        <div class="form-group" id="door_sizes" style="display:none;">
                                                            ...
                                                            <div class="row">
                                                                ...
                                                                <div class="swing_type_selection" id="swing_type_selection" style="display:none;">
                                                                    <div class="form-group">
                                                                ...
                                                                <div class="radio">
                                                                    <label>
                                                                        <input class="form-check-input" type="radio" name="swing_type_items" id="swing_type_items"
                                                                               value="{{ $item->adon_item_id }}" data-price="{{ $item->adon_item_price }}" data-adon_item_name="{{ $item->adon_item_name }}">
                                                                        {{ $item->adon_item_name }}
                                                                    </label>
                                                                </div>
                                                            @endforeach
...
<script>
    ...
    function updateGrandChildColors(childColorId) {

        // Send an AJAX request to the server to get the grandchild adons
        $.ajax({
            type: "GET",
            url:"/get-grandchild-colors/" + childColorId,
              beforeSend: function(xhr) {
                xhr.setRequestHeader('X-CSRF-Token', csrfToken);
            },
            success: function(data) {
                // Remove any previously displayed grandchild adons
                $('#grandChildColorContainer').empty();
                    $('#grandChildColorContainer').append('<strong>Color Items</strong>');

                // Loop through the grandchild adons and display them as radio buttons
                $.each(data, function(key, grandChildColor) {
                    $('#grandChildColorContainer').append('<div><input class="grandchild-color" type="radio" name="grandchildColor" value="'
                        + grandChildColor.adon_item_id + '"><label>' + grandChildColor.adon_item_name + '</label></div>');
                });
            },
...
<script>
    ...
    $(document).ready(function() {
        //showing the door types
        $(document).on('click', '.grandchild-color', function() {
...
//SELECTINF THE SIZE AGAINST THE DOOR TYPE SEPCIC!
        $('input[name="door_type"]').change(function() {
            var doorTypeId=$(this).val();
var WebURL = {!! json_encode(url('/')) !!}
            // Send an AJAX request to retrieve the available sizes for the selected door type

            $.ajax({
                url:WebURL +'/door/sizes/'+ doorTypeId,
                type: 'GET',
                ...
                success: function (data){
                    ...
                    $.each(data, function(index,sizes){
                        sizeOptions+='<div class="door_size">';
                        sizeOptions+='<label>';
                        sizeOptions+='<input type="radio" name=door_size value="' + sizes.adon_item_id + '" data-price="' + sizes.adon_item_price + '">';
                        sizeOptions += sizes.adon_item_name;
                        sizeOptions+='</label>';
                        sizeOptions+='</div>';

                    });
                    $('#door_sizes .col-sm-4').html(sizeOptions);
        ...
        $(document).on('click', '.door_size', function() {
            // Show the div if it's currently hidden
            $('#swing_type_selection').show();
        });
...
// Applying borders when an input type is checked
        $('input[name="door_type"]').change(function() {
            if($(this).is(':checked')) {
                $('label img').removeClass('checked');
                $(this).siblings('label').find('img').addClass('checked');
            } else {
                $(this).siblings('label').find('img').removeClass('checked');
            }
        });
